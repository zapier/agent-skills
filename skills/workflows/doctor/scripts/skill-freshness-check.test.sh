...
# ABOUTME: Uses env hooks (fake clock, stub update cmd, temp XDG_CACHE_HOME) — no network, no real time.
#
...
teardown() { rm -rf "$XDG_CACHE_HOME"; }

put_marker() { # last_success last_attempt failures version
  mkdir -p "$CACHE_DIR"
  printf '%s\n%s\n%s\n%s\n' "$1" "$2" "$3" "$4" > "$MARKER"
}
...
echo "Case 1: missing marker -> update changed -> updated + note"
...
setup; put_marker $((NOW-3600)) $((NOW-3600)) 0 "1.2.0"; run "$STUB_CHANGED"; want_state skipped; want_stdout_empty; want_rc0; teardown

echo "Case 4: bursting, attempt 10m ago -> skipped (cooldown)"
setup; put_marker $((NOW-90000)) $((NOW-600)) 1 "1.2.0"; run "$STUB_CHANGED"; want_state skipped; want_stdout_empty; teardown

echo "Case 5: bursting, attempt 20m ago -> due -> failure, count=2, silent"
setup; put_marker $((NOW-90000)) $((NOW-1200)) 1 "1.2.0"; run "$STUB_FAIL"; want_state failed; want_stdout_empty; want_rc0; want_marker_line 2 "$NOW"; want_marker_line 3 "2"; teardown

echo "Case 6: exhausted, attempt 2h ago -> skipped (daily fallback)"
setup; put_marker $((NOW-200000)) $((NOW-7200)) 3 "1.2.0"; run "$STUB_CHANGED"; want_state skipped; want_stdout_empty; teardown

echo "Case 7: exhausted, attempt 25h ago -> due -> success resets count"
setup; put_marker $((NOW-200000)) $((NOW-90000)) 3 "1.2.0"; run "$STUB_UNCHANGED"; want_state noop; want_rc0; want_marker_line 1 "$NOW"; want_marker_line 3 "0"; teardown
...
setup; nonrepo="$(mktemp -d)"; ( cd "$nonrepo" && ZAPIER_WORKFLOWS_DEBUG=1 ZAPIER_WORKFLOWS_DOCTOR_NOW="$NOW" ZAPIER_WORKFLOWS_DOCTOR_UPDATE_CMD="$STUB_UNCHANGED" bash "$SCRIPT" >/dev/null 2>&1 ); rc=$?; [ "$rc" -eq 0 ] && ok "non-repo exit 0" || bad "non-repo exit $rc"; rm -rf "$nonrepo"; teardown
